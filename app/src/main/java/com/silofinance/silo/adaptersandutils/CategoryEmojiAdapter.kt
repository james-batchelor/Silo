...
import com.silofinance.silo.databinding.ListItemCategoryEmojiBinding
import com.silofinance.silo.db.Category


/** So this ListAdapter extender takes an Category as an argument, and uses a generic ViewHolder defined in this same file at the end. ListAdapter is similar to
 * RecyclerView.Adapter, but implements getItemCount()/getItem()/etc on its own (and submitList() over in the fragment) */
class CategoryEmojiAdapter(private val selectCategoryClickListener: (cId: Long) -> Unit) :
    ListAdapter<Category, CategoryEmojiAdapter.ViewHolder>(CategoryDiffCallback()) {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.bind(item, selectCategoryClickListener)  // Defined below
    }


    /** A ViewHolder contains views and data, but also metadata used by a RecyclerView for correct positioning, animation, etc */
    class ViewHolder private constructor(val binding: ListItemCategoryEmojiBinding) : RecyclerView.ViewHolder(binding.root) {

        /** Bind data to the item (ie from a database). See the block comment in CategoryUtils.kt for more info */
        fun bind(item: Category, selectCategoryClickListener: (cId: Long) -> Unit) {
            binding.category = item
            binding.liceEmoji.setOnClickListener { selectCategoryClickListener(item.cId) }
        ...
        companion object {  // Needs to be in a companion object so it can be called on the ViewHolder class, not called on a ViewHolder instance
            ...
            fun from(parent: ViewGroup): ViewHolder {
                ...
                val binding = ListItemCategoryEmojiBinding.inflate(layoutInflater, parent, false)

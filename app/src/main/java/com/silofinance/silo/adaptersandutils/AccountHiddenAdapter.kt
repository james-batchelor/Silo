...
import com.silofinance.silo.databinding.ListItemAccountHiddenBinding
...
class AccountHiddenAdapter(
    private val mergeClickListener: (aId: Long) -> Unit,
    private val activateClickListener: (account: Account) -> Unit,
    private val deleteClickListener: (aId: Long) -> Unit,
    private val textClickListener: (aId: Long, aEmoji: String, aName: String) -> Unit,
) : ListAdapter<Account, AccountHiddenAdapter.ViewHolder>(AccountHiddenDiffCallback()) {
    ...
    class ViewHolder private constructor(val binding: ListItemAccountHiddenBinding) : RecyclerView.ViewHolder(binding.root) {

        /** Bind data to the item (ie from a database). See the block comment in AccountUtils.kt for more info */
        fun bind(
            item: Account,
            mergeClickListener: (aId: Long) -> Unit,
            activateClickListener: (account: Account) -> Unit,
            deleteClickListener: (aId: Long) -> Unit,
            textClickListener: (aId: Long, aEmoji: String, aName: String) -> Unit,
        ) {
            binding.account = item

            binding.liahEmoji.setOnClickListener { textClickListener(item.aId, item.aEmoji, item.aName) }
            binding.liahName.setOnClickListener { textClickListener(item.aId, item.aEmoji, item.aName) }
            binding.liahMerge.setOnClickListener { mergeClickListener(item.aId) }
            binding.liahActivate.setOnClickListener { activateClickListener(item) }
            binding.liahDelete.setOnClickListener { deleteClickListener(item.aId) }
        ...
        companion object {  // Needs to be in a companion object so it can be called on the ViewHolder class, not called on a ViewHolder instance
            ...
            fun from(parent: ViewGroup): ViewHolder {
                ...
                val binding = ListItemAccountHiddenBinding.inflate(layoutInflater, parent, false)
    ...
    class AccountHiddenDiffCallback : DiffUtil.ItemCallback<Account>() {

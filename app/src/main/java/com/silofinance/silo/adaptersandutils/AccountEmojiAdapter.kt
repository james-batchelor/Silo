...
import com.silofinance.silo.databinding.ListItemAccountEmojiBinding
...
class AccountEmojiAdapter(private val selectAccountClickListener: (aId: Long) -> Unit) :
    ListAdapter<Account, AccountEmojiAdapter.ViewHolder>(AccountDiffCallback()) {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.bind(item, selectAccountClickListener)  // Defined below
    }


    /** A ViewHolder contains views and data, but also metadata used by a RecyclerView for correct positioning, animation, etc */
    class ViewHolder private constructor(val binding: ListItemAccountEmojiBinding) : RecyclerView.ViewHolder(binding.root) {

        /** Bind data to the item (ie from a database). See the block comment in AccountUtils.kt for more info */
        fun bind(item: Account, selectAccountClickListener: (aId: Long) -> Unit) {
            binding.account = item
            binding.liaeEmoji.setOnClickListener { selectAccountClickListener(item.aId) }
        ...
        companion object {  // Needs to be in a companion object so it can be called on the ViewHolder class, not called on a ViewHolder instance
            ...
            fun from(parent: ViewGroup): ViewHolder {
                ...
                val binding = ListItemAccountEmojiBinding.inflate(layoutInflater, parent, false)

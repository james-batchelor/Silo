...
class SleepDatabaseTest {
    ...
    private lateinit var dao: TransactionDbDao
    private lateinit var db: TransactionDb

    @Before
    fun createDb() {
        val context = InstrumentationRegistry.getInstrumentation().targetContext
        // Using an in-memory (not file system) database so the information stored here disappears when the process is killed

        db = Room.inMemoryDatabaseBuilder(context, TransactionDb::class.java)
            // Allowing main thread queries, just for testing.
            .allowMainThreadQueries()
            .build()
        dao = db.transactionDbDao
    }
    ...
    fun insertAndGetNight() {
        ...
        val lastTransaction = dao.get(1)
        assertEquals(lastTransaction.tNote, "")
    }
}